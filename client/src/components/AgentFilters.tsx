...
export function AgentFilters({ plan }: { plan: PlanResponse }) {
  ...
  return <section className="agent-filters" aria-label="Interpreted search preferences" aria-live="polite">
    <div className="agent-filters-heading"><strong>What I’m looking for</strong><span>{plan.agent.mode === 'model' ? 'AI interpretation' : 'Local interpretation'} · {plan.dataSource === 'verified-demo-data' ? 'Demo catalog' : 'Provider catalog'}</span></div>
