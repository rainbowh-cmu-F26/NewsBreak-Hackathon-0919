...
const quickSearches: Array<{ label: string } & PlanRequest> = [
  { label: 'Compare all 15', prompt: 'All restaurants', budget: 35, dietary: 'No preference' },
...
const platformLabel = (id: string) => ({ ubereats: 'Uber Eats', doordash: 'DoorDash', restaurant_direct: 'Restaurant delivery' }[id] || id);
...
function FoodPhoto({ item, large = false }: { item: string; large?: boolean }) {
  const [failed, setFailed] = useState(false);
  return <div className={`food-photo ${large ? 'food-photo-large' : ''}`}>
    {!failed ? <img src={foodPhoto(item)} alt={`Illustrative food photography for ${item}; not the restaurant's actual dish`} loading={large ? 'eager' : 'lazy'} onError={() => setFailed(true)} /> : <div className="food-photo-fallback"><Utensils size={42}/><span>Photo unavailable</span></div>}
...
function OfferDialog({ offer, saved, onSave, onClose }: { offer: DeliveryOption; saved: boolean; onSave: () => void; onClose: () => void }) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="offer-dialog" role="dialog" aria-modal="true" aria-labelledby="offer-title"><FoodPhoto item={offer.item} large/><button className="dialog-close" onClick={onClose} aria-label="Close offer details"><X size={18} /></button><span className="deal-badge">{offer.badge}</span><p className="section-kicker dialog-kicker">QUOTE DETAILS</p><h2 id="offer-title">{restaurantName(offer.restaurant)}</h2><p className="dialog-item">{offer.item}</p><p className="photo-disclosure">Food photography is illustrative and does not show this restaurant’s actual dish or storefront.</p><p className="dialog-detail">{offer.detail}</p>{offer.sourceUrl && <a href={offer.sourceUrl} target="_blank" rel="noreferrer">View source on platform ↗</a>}<div className="dialog-price"><span>{offer.quote?.dataType === 'menu_only' ? 'Menu price · before fees' : offer.quote?.dataType === 'synthetic' ? 'Simulated total · not a live quote' : 'Recorded total'}</span><strong>${(offer.price + offer.fee).toFixed(2)}</strong></div><div className="dialog-facts"><span><Clock3 size={16} /><strong>{offer.eta}</strong><small>estimated arrival · demo when simulated</small></span><span><BadgeCheck size={16} /><strong>{offer.quote?.dataType === 'menu_only' ? 'Public menu' : offer.verified ? 'Snapshot checked' : 'Simulated'}</strong><small>{offer.quote?.capturedAt ? new Date(offer.quote.capturedAt).toLocaleString() : "No live capture"}</small></span></div>{offer.quote && <dl>{[
