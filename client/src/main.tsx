...
const startingPlan: PlanResponse = { summary: 'Your shortlist will appear here.', options: [], savings: 0, checkedAt: new Date().toISOString(), dataSource: 'database-data' };
const apiBaseUrl = import.meta.env.VITE_API_URL || 'http://localhost:8787';
const quickSearches: Array<{ label: string } & PlanRequest> = [
  { label: 'Browse restaurants', prompt: 'All restaurants', budget: 35, dietary: 'No preference' },
...
function App() {
  ...
  function focusChat() {
    chatRegion.current?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth', block: 'center' });
    chatRegion.current?.querySelector('input')?.focus({ preventScroll: true });
  }
  useEffect(() => { if (chatOpen) focusChat(); }, [chatOpen]);
  function openChat() { setChatOpen(true); if (chatOpen) focusChat(); }


  const visibleOptions = useMemo(() => showSavedOnly ? savedRestaurants : browsing ? plan.options : uniqueRestaurants(plan.options), [plan.options, savedRestaurants, showSavedOnly, browsing]);

  async function loadCatalog(signal?: AbortSignal) {
    const version = ++requestVersion.current;
    setLoading(true); setError(''); setBrowsing(true); setShowSavedOnly(false);
    try {
      const catalog = await fetchCatalog(signal);
      if (version === requestVersion.current && !signal?.aborted) setPlan(catalog);
    } catch (caught) {
      if (version === requestVersion.current && !signal?.aborted) setError(caught instanceof Error ? caught.message : 'Could not load the catalog.');
    } finally {
  ...
  async function submit(event?: FormEvent, nextValues?: PlanRequest) {
    ...
    try {
      ...
      const result = await fetchPlan(values);
      if (version !== requestVersion.current) return;
      setPlan(result);
      window.requestAnimationFrame(() => document.getElementById('shortlist')?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
    } catch (caught) {
      if (version === requestVersion.current) setError(caught instanceof Error ? caught.message : 'Something went wrong.');
    } finally { if (version === requestVersion.current) setLoading(false); }
  }

  function applyQuickSearch(search: typeof quickSearches[number]) {
    if (search.prompt === 'All restaurants') { void loadCatalog(); return; }
...
const platformLabel = (id: string) => ({ grubhub: 'Grubhub', ubereats: 'Uber Eats', 'uber-eats': 'Uber Eats', 'restaurant-direct': 'Restaurant delivery', doordash: 'DoorDash', restaurant_direct: 'Restaurant delivery' }[id] || id);
